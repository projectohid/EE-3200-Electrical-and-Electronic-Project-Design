...
if (now > 1700000000) return; // time synced
delay(200);
}
}
...
{
ma_window[ma_i] = x; ma_i = (ma_i + 1) % 5;
long s = 0; for (int i = 0; i < 5; ++i) s += ma_window[i];
return (int16_t)(s / 5);
}
...
if (delta < 0) return; // not time yet
nextUs += periodUs;
...
for (int i = 0; i < CHUNK; ++i) arr.add((int)buf[i]);
